...
class RelayController:
    ...
    def emergency_off(self) -> None:
        """
        Размыкает реле максимально быстро: команда уходит в порт, ответ не
        вычитывается вовсе.

        Штатный off() ждёт от платы 'OK' до READ_MAX_WAIT (1.5 с) — в
        аварийной ситуации это ожидание не даёт ничего: команда либо уже
        ушла в порт, либо порт мёртв и никакое ожидание его не оживит. Зато
        полторы секунды под током — это полторы секунды под током.

        Ничего не возвращает: возвращать было бы нечего, кроме выдумки.
        ...
        try:
            ...
            # flush уже не критичен: байты в порт отданы. Молча глотаем,
            # чтобы не спровоцировать в вызывающем коде обрыв порта там,
            # где команда на самом деле ушла.
            pass

    def drop(self) -> None:
        """
        Принудительно закрывает порт, не пытаясь ничего отправить.

        Последнее средство, когда плата не приняла даже emergency_off():
