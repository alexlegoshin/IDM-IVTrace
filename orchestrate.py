...
def write_results_csv(csv_path: Path, df: pd.DataFrame, params: dict,
    ...
    with open(csv_path, 'w', encoding='utf-8') as f:
        ...
        if params.get('use_relay', True) is False:
            f.write("# Режим: измерение без реле (одна полярность)\n")
        else:
            f.write("# Режим: измерение с переключением полярности через реле\n")
        if params.get('invert_input', False):
            f.write("# Входные значения инвертированы (перепутана полярность)\n")
...
def run_measurement_session(
    ...
    # Получаем новые параметры из params
    ...
    invert_input = params.get('invert_input', False)
