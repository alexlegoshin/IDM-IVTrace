...
# Плавное нарастание (feature "смягчённый пуск тока", BETA) — порог, ВЫШЕ
# которого доверия к алгоритму сглаживания меньше, чем к простому мгновенному
# скачку (не подтверждён на реальном стенде на больших токах). Баг-репорт:
# раньше это был ЖЁСТКИЙ потолок (измерение не запускалось выше 300 А). По
# требованию заказчика теперь это лишь ПРЕДУПРЕЖДЕНИЕ — ампераж не ограничен,
# ответственность за режим на больших токах на операторе. Жёсткие пределы —
# только у реле (RELAY_MAX_CURRENT_A) и источника (паспорт), они в силе.
SMOOTH_RAMP_WARN_CURRENT_A = 300.0


def smooth_ramp_warning(max_abs_current_a: Optional[float]) -> Optional[str]:
    """
    Возвращает предупреждающий текст, если плавное нарастание запрошено для
    развёртки с максимальным током выше SMOOTH_RAMP_WARN_CURRENT_A, иначе
    None. НЕ блокирует измерение (баг-репорт: раньше блокировало) — только
    предупреждает. Не имеет отношения к возбуждению напряжением — вызывающая
    сторона проверяет это только для excitation_type == 'current' и только
    когда сам режим включён.
    """
    if max_abs_current_a is None:
        return None
    if max_abs_current_a > SMOOTH_RAMP_WARN_CURRENT_A:
        return (
            f"Плавное нарастание (BETA) выше {SMOOTH_RAMP_WARN_CURRENT_A:.0f} А "
            f"(запрошено {max_abs_current_a:.1f} А) — алгоритм сглаживания на таких токах "
            "не проверялся на реальном стенде, ответственность на операторе."
        )
    return None
