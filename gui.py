...
class IVTraceGUI:
    ...
    def _build_params(self, parent):
        ...
        self.invert_input_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opts, text="Инвертировать входные значения (если перепутана полярность)",
                        variable=self.invert_input_var).grid(row=3, column=0, columnspan=2, sticky="w")
    ...
    def _save_config(self):
        ...
        params['invert_input'] = self.invert_input_var.get()
    ...
    def _load_config(self):
        ...
        self.invert_input_var.set(params.get('invert_input', False))
    ...
    def _gather_params(self):
        ...
        try:
            ...
            # Проверяем, что I_nom и ratio заданы, если включена остановка по погрешности
            if params["stop_on_error"]:
                if params["I_nom"] is None:
                    raise ValueError("Для остановки по погрешности необходимо указать I ном.")
                if params["ratio"] is None:
                    raise ValueError("Для остановки по погрешности необходимо указать коэффициент.")
